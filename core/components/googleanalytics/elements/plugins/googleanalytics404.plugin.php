	...
	switch($modx->event->name) {
		case 'OnPageNotFound':
			if ('mgr' != $modx->context->key) {
				require_once $modx->getOption('googleanalytics.core_path', null, $modx->getOption('core_path').'components/googleanalytics/').'/model/googleanalytics/googleanalytics.class.php';

				$googleAnalytics = new GoogleAnalytics($modx);
				$googleAnalytics->setGoogleAnalytics404();
			}
