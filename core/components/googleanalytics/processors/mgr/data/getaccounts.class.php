<?php
	...
	class SocialMediaSourcesGetAccountsProcessor extends modProcessor {
		/**
		 * @access public.
		 * @var Array.
		 */
		...
		public function process() {
			...
			if (null !== ($data = $this->googleanalytics->getRequest()->getAccounts())) {
				if (isset($data['items'])) {
					foreach ($data['items'] as $key => $value) {
						...
						if (!empty($query)) {
							if (false !== strstr(strtolower($value['name']), strtolower($query))) {
								$output[] = $value;
							}
						} else {
							$output[] = $value;
	...
	return 'SocialMediaSourcesGetAccountsProcessor';
